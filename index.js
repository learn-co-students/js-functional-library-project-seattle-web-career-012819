	...
	return {
		...
		each: function(collection, iteratee) {
			...
			for (let idx = 0; idx < newCollection.length; idx++) iteratee(newCollection[idx]);
		...
		map: function(collection, iteratee) {
			...
			const newArr = [];
			for (let i = 0; i < collection.length; i++) newArr.push(iteratee(collection[i]));
			return newArr;
		},
		...
		find: function(collection, predicate) {
			...
			for (let idx = 0; idx < collection.length; idx++) if (predicate(collection[idx])) return collection[idx];
		...
		flatten: function(collection, shallow, newArr = []) {
			...
			if (shallow) {
				for (let val of collection) Array.isArray(val) ? this.unpack(newArr, val) : newArr.push(val);
			} else {
